...
let isPipingToUserInput = false;  // Initialize the variable

let audioBuffer = []; // Buffer for holding audio data
let chunkInterval = 60000; // Rotate the chunks every 60 seconds (you can adjust this)
...
function initializeAssistant() {
    if (isAlwaysListening) {
        // Play greeting and ask for immediate tasks
        speakText("Hello! Welcome back. Are there any immediate tasks I can assist you with?");
        
        // Automatically start recording and scanning for voice commands
        navigator.mediaDevices.getUserMedia({ audio: true })
            .then((stream) => {
                mediaStream = stream;
                console.log('Microphone initialized for always listening mode');
                startRecording(stream);  // Start recording automatically
            })
            .catch((error) => {
...
function speakText(text, settings = {}) {
    ...
    const selectedVoiceIndex = voiceSelect ? voiceSelect.value : null;  // Get the selected voice's index if available
    const voices = speechSynthesis.getVoices();

    const defaultSettings = {
        language: 'en-US',  // Default to English (US) if no selection
        rate: 1.0,          // Adjusted the default rate to a more neutral 1.0
        pitch: 1.0          // Default pitch to 1.0
    };
    ...
    // Use the selected voice if available, else fallback to the first available voice or default voice
    if (voices.length > 0) {
        // If a specific voice is selected, use it
        if (selectedVoiceIndex && voices[selectedVoiceIndex]) {
            utterance.voice = voices[selectedVoiceIndex];
        } else {
            // Use the default voice (or the first available voice if no specific selection is made)
            utterance.voice = voices.find(voice => voice.default) || voices[0];
            console.log("Using default or first available voice:", utterance.voice.name);
        }
    } else {
        console.log("No voices available, using browser default voice.");
    }
...
window.onload = function() {
    const isAlwaysListening = loadListeningState();
    if (isAlwaysListening) {
        initializeAssistant();
    }
};
...
let continuousBuffer = []; // Buffer for real-time transcription
...
function startRecording(stream) {
    ...
    mediaRecorder.ondataavailable = function(event) {
        audioChunks.push(event.data); // Collect audio chunks for saving
        continuousBuffer.push(event.data); // Push to buffer for real-time processing
    };

    // Start chunk rotation and real-time transcription
    startChunkRotation();
    transcribeInRealTime(continuousBuffer); // Process real-time transcription
    isRecording = true;
    micButton.style.backgroundColor = 'green';  // Indicate recording
    listeningStatus.textContent = isAlwaysListening ? 'Listening...' : 'Recording...';

    // Start updating the volume meter
    updateVoiceMeterState(stream);

    // When the recording stops
    mediaRecorder.onstop = function() {
        isRecording = false;
        micButton.style.backgroundColor = 'red';  // Indicate stopped recording
...
function stopRecording() {
    if (mediaRecorder && mediaRecorder.state === 'recording') {
        ...
        isRecording = false;
        micButton.style.backgroundColor = 'red';  // Indicate stopped recording
        listeningStatus.textContent = 'Listening...';  // Reset status to default
    }
}

// Function to rotate chunks every 60 seconds without interrupting recording
function startChunkRotation() {
    chunkTimeout = setInterval(function () {
        processAndSaveChunk(); // Process and save the last 60 seconds of audio
        audioChunks = []; // Clear the chunk array for new data
        console.log('Chunk rotation completed, starting new chunk...');
    }, chunkInterval);
}

// Process and save the current chunk of audio to the backend
function processAndSaveChunk() {
    ...
    .then(data => {
        if (data.transcribed_text) {
            appendTranscription(data.transcribed_text); // Append transcribed text
            handleVoiceCommands(data.transcribed_text);  // Handle voice commands
        }
    })
    .catch(error => console.error('Error processing audio chunk:', error));
}

// Real-time transcription process for continuous audio buffer
function transcribeInRealTime(audioBuffer) {
    const realTimeBlob = new Blob(audioBuffer, { type: 'audio/webm' });
    const formData = new FormData();
    formData.append('audio', realTimeBlob);

    // Send the buffered audio to the backend for real-time transcription
    fetch('/transcribe_realtime', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
    .then(data => {
        if (data.transcribed_text) {
            appendTranscription(data.transcribed_text); // Append to input field
        }
    })
    .catch(error => console.error('Real-time transcription error:', error));
}

// Function to initialize the microphone and start listening or recording based on the toggle
function initializeMicrophone() {
    const isAlwaysListening = loadListeningState();
    alwaysListeningToggle.checked = isAlwaysListening;

    if (isAlwaysListening) {
        // Start the microphone and begin always listening
        navigator.mediaDevices.getUserMedia({ audio: true })
            .then((stream) => {
                mediaStream = stream;
                startRecording(mediaStream);  // Start recording the stream
                console.log('Microphone initialized for always listening');
            })
            .catch(error => {
                console.error('Error accessing microphone: ', error);
        ...
        stopRecording();  // Stop recording when "always listening" is off
    }

    alwaysListeningToggle.addEventListener('change', function() {
        const isAlwaysListening = this.checked;
        saveListeningState(isAlwaysListening);

        if (isAlwaysListening) {
            // Reinitialize the microphone and start recording
            navigator.mediaDevices.getUserMedia({ audio: true })
                .then((stream) => {
                    mediaStream = stream;
                    startRecording(mediaStream);  // Start listening again
                })
                .catch(error => {
                    console.error('Error accessing microphone: ', error);
                });
...
// Helper function to append transcribed text to chat or task input
...
// Function to update the voice meter state (volume visualization)
function updateVoiceMeterState(stream) {
    ...
    function updateMeter() {
        ...
        document.getElementById('volume-bar').style.width = volumePercentage + '%';  // Update the volume meter bar

        if (stream.active) {
            requestAnimationFrame(updateMeter);  // Continue to update while the stream is active
        }
    }
...
// Trigger greeting and listen for initial commands
function initializeGreeting() {
    if (isAlwaysListening) {
        speakText('Hello! How can I assist you today? Please speak a command or ask a question.');
        startRecording(mediaStream);
    }
}


// Logic for managing mic button based on the isAlwaysListening state
micButton.addEventListener('click', function (e) {
    e.preventDefault();
    
    if (isAlwaysListening) {
        // If always listening, toggle piping the transcribed text to chat input
        if (!isPipingToUserInput) {
            isPipingToUserInput = true;
            listeningStatus.textContent = 'Piping text to user input...';
            micButton.style.backgroundColor = 'green';  // Indicate piping started
        } else {
            isPipingToUserInput = false;
            listeningStatus.textContent = 'Listening...';
            micButton.style.backgroundColor = 'red';  // Indicate piping stopped
        }
    } else {
...
window.onload = function () {
    ...
    if (isAlwaysListening) {
        startRecording(mediaStream);  // This should initialize the microphone and volume meter
    } else {
...
document.getElementById('user-input').addEventListener('input', function () {
    ...
    document.getElementById('read-response-button').addEventListener('click', function() {
        ...
        if (llmResponse) {
            speakText(llmResponse);  // speakText function should be properly defined to use speechSynthesis.
        } else {
            speakText('Sorry, but there is no response available to read.');
        }
    });
...
function selectPipeline() {
    ...
    // Call this function when the voices are updated to ensure the dropdown is populated
    function populateVoiceList() {
        const voiceSelect = document.getElementById('voice-selection');
        if (!voiceSelect) return;  // Ensure voice selection element exists
        ...
        // Set the first voice as default if no previous selection
        const savedVoiceIndex = localStorage.getItem('selectedVoice') || 0;
        voiceSelect.value = savedVoiceIndex;
    }

    // Ensure the voices are loaded, and update the voice selection dropdown when the voices change
...
// Submit new microphone and voice settings
document.getElementById('save-settings-button').addEventListener('click', function() {
    ...
    // Save the microphone settings in localStorage
    localStorage.setItem('selectedMicrophone', selectedMicrophone);
    localStorage.setItem('selectedVoice', selectedVoice);
    localStorage.setItem('voiceLanguage', selectedLanguage);
    localStorage.setItem('voiceRate', selectedRate);
    ...
    // Reinitialize the microphone with the new settings
    navigator.mediaDevices.getUserMedia({ audio: { deviceId: { exact: selectedMicrophone } } })
        .then((stream) => {
            mediaStream = stream;
            startRecording(mediaStream);  // Start listening with the new settings
            console.log('Microphone settings updated and stream reinitialized.');
        })
        .catch(error => {
            console.error('Error accessing microphone with new settings: ', error);
            alert('Could not access the selected microphone.');
        });
