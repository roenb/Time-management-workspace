...
# Path to the JSON file where reflection data is stored
DATA_FILE_PATH = 'data/reflection_data.json'

# Path to the JSON file where task data is stored
...
# Helper function to update in-memory data for insights
    ...
    if os.path.exists(TASKS_FILE_PATH):
        try:
            with open(TASKS_FILE_PATH, 'r') as file:
                data = file.read().strip()  # Read and remove any surrounding whitespace
                if not data:
                    # File is empty, return a default structure
                    return {"tasks": []}
                return json.loads(data)
        except json.JSONDecodeError:
            # If the file is corrupted or contains invalid JSON, return an empty structure
            return {"tasks": []}
    else:
        return {"tasks": []}
...
def add_task():
    ...
    # Add the new task
    tasks_data['tasks'].append({"task": task, "completed": False})
                ...
                if chunk:
                    ...
                    for line in lines[:-1]:  # Process all lines except the last incomplete one
                        ...
                        if line.startswith("data:"):
                            token = line[5:].strip()  # Remove the "data: " prefix
...
def read_mantra_text(doc_path):
    """Reads the text from the provided Word document."""
